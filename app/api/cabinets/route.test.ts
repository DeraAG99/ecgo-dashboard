...
describe("GET /api/cabinets", () => {
  ...
  it("should return paginated cabinet list", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/cabinets"))
  ...
  it("should return 400 for invalid status", async () => {
    const res = await GET(new NextRequest("http://localhost/api/cabinets?status=INVALID"))
  ...
  it("should apply search, status, sort to the query", async () => {
    ...
    await GET(
      new NextRequest(
        "http://localhost/api/cabinets?search=CB&status=ONLINE&sortBy=code&sortOrder=asc&page=2&limit=5"
      )
    )
