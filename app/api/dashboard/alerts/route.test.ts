...
describe("GET /api/dashboard/alerts", () => {
  ...
  it("should return paginated alert list with unread count", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/alerts?limit=20"))
  ...
  it("should apply read and severity filters", async () => {
    ...
    const res = await GET(
      new NextRequest("http://localhost/api/dashboard/alerts?read=false&severity=CRITICAL")
    )
    expect(res.status).toBe(200)
  ...
  it("should return 400 for invalid query", async () => {
    const res = await GET(new NextRequest("http://localhost/api/dashboard/alerts?type=INVALID"))
  ...
  it("should return 500 on db error", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/alerts"))
    expect(res.status).toBe(500)
  })
})

describe("POST /api/dashboard/alerts", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
...
describe("PATCH /api/dashboard/alerts (mark all read)", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
