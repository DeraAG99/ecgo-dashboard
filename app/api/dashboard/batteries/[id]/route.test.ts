...
describe("GET /api/dashboard/batteries/[id]", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("should return battery detail with swap history", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/batteries/bat-0001"))
  ...
  it("should return 404 when battery not found", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/batteries/NOPE"))
