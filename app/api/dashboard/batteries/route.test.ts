...
describe("GET /api/dashboard/batteries", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("should return paginated battery list", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/batteries?limit=20"))
  ...
  it("should apply status and minHealth filters", async () => {
    ...
    const res = await GET(
      new NextRequest("http://localhost/api/dashboard/batteries?status=RETIRED&minHealth=20")
    )
    expect(res.status).toBe(200)
  ...
  it("should return 400 for invalid status", async () => {
    const res = await GET(new NextRequest("http://localhost/api/dashboard/batteries?status=INVALID"))
    expect(res.status).toBe(400)
    expect(dbMock.select).not.toHaveBeenCalled()
  })

  it("should return 400 for invalid minHealth range", async () => {
    const res = await GET(new NextRequest("http://localhost/api/dashboard/batteries?minHealth=150"))
    expect(res.status).toBe(400)
  })
  ...
  it("should return 500 on db error", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/batteries"))
