...
describe("GET /api/dashboard/cabinets/[id]", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("should return cabinet detail with slots, history and 24h chart", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/cabinets/CB-001"))
  ...
  it("should return 404 when cabinet not found", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/cabinets/NOPE-999"))
