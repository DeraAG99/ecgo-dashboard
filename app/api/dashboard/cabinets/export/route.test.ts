...
describe("GET /api/dashboard/cabinets/export", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("should return CSV with header and rows", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/cabinets/export"))
  ...
  it("should filter rows by status param", async () => {
    ...
    await GET(new NextRequest("http://localhost/api/dashboard/cabinets/export?status=OFFLINE"))
