...
describe("POST /api/dashboard/checkins", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("should return VALID when location is inside cabinet radius", async () => {
    ...
    const res = await POST(
      new NextRequest("http://localhost/api/dashboard/checkins", {
        method: "POST",
...
describe("GET /api/dashboard/checkins", () => {
  const row = {
    id: "ci-abc123",
    userId: "U-0001",
    lat: -6.2001,
  ...
  it("should return paginated check-in history", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/checkins"))
  ...
  it("should return 400 for invalid result filter", async () => {
    const res = await GET(
      new NextRequest("http://localhost/api/dashboard/checkins?result=INVALID")
    )
    expect(res.status).toBe(400)
    expect(dbMock.execute).not.toHaveBeenCalled()
  })
  ...
  it("should apply user and result filters", async () => {
    ...
    await GET(
      new NextRequest(
        "http://localhost/api/dashboard/checkins?userId=U-0001&result=VALID&page=2&limit=10"
      )
    )
