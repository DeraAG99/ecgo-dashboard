...
describe("GET /api/dashboard/transactions", () => {
  ...
  it("should return transactions with total", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/transactions?limit=2"))
  ...
  it("should filter by cabinetId", async () => {
    ...
    await GET(new NextRequest("http://localhost/api/dashboard/transactions?cabinetId=CB-002"))
  ...
  it("should accept startDate and endDate filters", async () => {
    ...
    const res = await GET(
      new NextRequest(
        "http://localhost/api/dashboard/transactions?startDate=2026-08-01&endDate=2026-08-15"
      )
    )
    expect(res.status).toBe(200)
    expect(dbMock.select).toHaveBeenCalledTimes(2)
  ...
  it("should return 400 for invalid date filter", async () => {
    const res = await GET(
      new NextRequest("http://localhost/api/dashboard/transactions?startDate=not-a-date")
    )
  ...
  it("should return 400 for negative limit", async () => {
    const res = await GET(new NextRequest("http://localhost/api/dashboard/transactions?limit=-1"))
    expect(res.status).toBe(400)
  })
  ...
  it("should return 500 on db error", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/dashboard/transactions"))
