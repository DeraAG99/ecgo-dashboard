...
describe("GET /api/maintenance/logs", () => {
  ...
  it("should return 400 for invalid query", async () => {
    const res = await GET(new Request("http://localhost/api/maintenance/logs?entityType=FOO") as never)
  ...
  it("should return paginated logs", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/maintenance/logs?page=1&limit=50") as never)
